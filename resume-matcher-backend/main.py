...
async def compare_texts(job_text: str, resume_text: str) -> dict:
    try:
        ...
        # c. Match Score
        match_score_prompt = (
            "Based on the resume and job posting comparison, calculate a percentage match score using this formula:\n"
            "Match Score = (Sum of weight_match_score) ÷ (Sum of weight_match_total) × 100\n\n"
            "Weight mapping:\n"
            "✅ Strong = 1.0\n"
            "✅ Moderate-Strong = 0.8\n"
            "⚠️ Partial = 0.5\n"
            "❌ Lack = 0.0\n\n"
            "Return ONLY the final calculated percentage number, no text, no symbols, no explanation."
        )
