...
async def compare_texts(job_text: str, resume_text: str) -> dict:
    try:
        ...
        resume_summary_prompt = (
            ...
            f"{job_summary}\n\n"
            "Output a comparison table. The comparison is between the job responsibilities, skills, certificates, and education requirements from job_summary_prompt outputs and the highlights of the user's skills and experiences in the upload resume contents. List in the table format with three columns: Categories (All the items listed from the requirements of position responsibilities and skills, certifications, and educations), Match Status (four status will be used: ✅ Strong (the item is also mentioned in the user's resume and very well-matched with what mentioned in job_summary_prompt) / 🔷 Moderate-strong (the item is also mentioned in the user's resume and closely matched with what mentioned in job_summary_prompt)/⚠️ Partial (the item is kind of mentioned in the user's resume and some parts matched with what mentioned in job_summary_prompt)/ ❌ Lack (the item is not clearly mentioned in the user's resume and only little bit or not match with what mentioned in job_summary_prompt)), and Comments (very precise comment on how the user's experiences matches with the job requirement). Make sure to output the table in HTML format, with <table>, <tr>, <th>, <td> tags, and do not add any explanation or extra text. The table should be styled to look clean and modern. Only output the table in HTML format, with <table>, <tr>, <th>, <td> tags, and do not add any explanation or extra text. The table should be styled to look clean and modern. Do not show the word ```html in output."
        )
        ...
        # c. Match Score
        match_score_prompt = (
            "Read the job_summary comparison table. Based on the listed Match Status for each category, calculate and return a single percentage match score. Use the following scoring rules: If the Match Status is Strong, then score_Strong =1, total_Strong =1. If the Match Status is Moderate-Strong, then score_Mod=0.8, total_Mod=1. If the Match Status is Partial, then score_Partial=0.5, total_Partial=1. If the Match Status is Lack, then score_Lack=0, total_Lack=1. The formula for calculating the match score is: Match Score (%) =(sum of all the score_Strong + sum of all the score_Mod + sum of all the score_Partial + sum of all the score_Lack) ÷ (sum of all the total_Strong + sum of all the total_Mod + sum of all the total_Partial + sum of all the total_Lack). Return only the final match score as a number, rounded to two decimal places. Do not include any symbols, explanations, or additional text in the output, no explanation, no symbols, no text. Do not show the word ```html in output."
        )
        ...
        tailored_resume_summary_prompt = (
            ...
            f"{job_text}\n\n"
            "Provide a revised one-paragraph summary based on the original summary in resume_text (the user's resume). If the user's resume does not have a summary section, write a new summary as the revised summary. Make sure this summary highlights the user's skills and work experience and makes it more closely match the job requirements in job_text. Please limit the overall summary to 1700 charactersble. The output should be in HTML format and should maintain a simple, modern style. Write this paragraph in the first person. Maintain 1.2 line spacing. Do not show the word ```html in output."
        )
