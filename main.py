...
async def compare_texts(job_text: str, resume_text: str) -> dict:
    try:
        ...
        if lines:
            ...
            try:
                match_score = float(last_line.replace("%", ""))
            except Exception:
                match_score = last_line
        else:
            match_score = None

        # c. Match Score
        match_score_prompt = match_score
